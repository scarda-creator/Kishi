Q.gauss = [
  ...
  { pool:"gauss", src:"tipo A · manuale", steps:[{
      ...
      hint:"La regolarita\\u0300 richiesta e\\u0300 $C^1$; la semplice connessione riguarda i campi conservativi, non Gauss.",
      sol:"Il Teorema della divergenza richiede $F\\in C^1$ su un intorno del compatto $\\overline V$ e $\\partial V$ regolare a tratti con normale $n$ USCENTE. La semplice connessione e\\u0300 irrilevante qui; $\\nabla\\cdot F$ puo\\u0300 essere una qualsiasi funzione continua."
    }]
  },
  ...
  { pool:"gauss", src:"tipo A · manuale", steps:[{
      ...
      sol:"$n$ e\\u0300 la normale USCENTE. Se usi quella entrante il flusso cambia segno: $\\iint_{\\partial V}F\\cdot n_{\\text{ent}}\\,dS=-\\iiint_V\\nabla\\cdot F\\,dV$. Teorema della divergenza."
    }]
  },
  ...
  { pool:"gauss", src:"tipo A · manuale", steps:[{
      ...
      hint:"L'integrale di una costante sul volume e\\u0300 la costante per il volume.",
      sol:"$\\iiint_V c\\,dV=c\\,\\operatorname{vol}(V)$. Quando la divergenza e\\u0300 costante il flusso si riduce a un volume: e\\u0300 il motivo per cui Gauss conviene. Teorema della divergenza."
    }]
  },

  { pool:"gauss", src:"tipo A · manuale", steps:[{
      t:"La frontiera $\\partial V$ e\\u0300 fatta di tre superfici e $\\nabla\\cdot F=2$. Conviene di piu\\u0300:",
      opts:[
      ...
      sol:"Con $\\nabla\\cdot F$ costante o semplice, Gauss trasforma tre flussi di superficie in un unico integrale di volume $\\iiint_V\\nabla\\cdot F$. E\\u0300 il criterio di convenienza. Teorema della divergenza."
    }]
  },

  { pool:"gauss", src:"tipo H · manuale", steps:[{
      t:"$\\Sigma$ e\\u0300 una superficie APERTA (una calotta senza coperchio). Per calcolarne il flusso con Gauss devi:",
      opts:[
  ...
  { pool:"gauss", src:"tipo H · manuale", steps:[{
      ...
      hint:"Nel bilancio di Gauss ogni pezzo di $\\partial V$ e\\u0300 orientato verso l'esterno.",
      sol:"Applicando Gauss al solido chiuso, sia $\\Sigma$ sia $T$ vanno con normale uscente da $V$. Isolando $\\Sigma$: $\\iint_\\Sigma=\\iiint_V\\nabla\\cdot F-\\iint_T$ con $T$ uscente. Sbagliarne il verso e\\u0300 l'errore classico del metodo. Teorema della divergenza."
    }]
  },

  { pool:"gauss", src:"tipo H · manuale", steps:[{
      t:"Su una superficie cartesiana $z=f(x,y)$ si ha $r_x\\wedge r_y=(-f_x,-f_y,1)$ (terza componente positiva, punta in alto). Se questa e\\u0300 il tappo INFERIORE di un solido, la normale uscente da $V$ e\\u0300:",
      opts:[
        {x:"$(f_x,f_y,-1)$, cioe\\u0300 verso il basso",c:true},
        {x:"$(-f_x,-f_y,1)$, invariata"},
        {x:"$(0,0,1)$"},
        {x:"$(0,0,-1)$"}],
      hint:"Sul fondo del solido, 'uscente' significa verso il basso.",
      sol:"$(-f_x,-f_y,1)$ punta in alto; sul tappo inferiore l'uscente da $V$ punta in basso, quindi si prende $(f_x,f_y,-1)$. Confondere i due segni e\\u0300 la trappola piu\\u0300 frequente. Teorema della divergenza."
    }]
  },

  { pool:"gauss", src:"20230210 Es.4", steps:[{
      t:"$\\partial V=\\Sigma_1\\cup\\Sigma_2\\cup\\Sigma_3$. Conosci il flusso totale (via Gauss) e i flussi su $\\Sigma_1,\\Sigma_2$. Il flusso su $\\Sigma_3$ e\\u0300:",
      opts:[
        {x:"$\\iiint_V\\nabla\\cdot F\\,dV-\\Phi_{\\Sigma_1}-\\Phi_{\\Sigma_2}$",c:true},
        {x:"$\\iiint_V\\nabla\\cdot F\\,dV+\\Phi_{\\Sigma_1}+\\Phi_{\\Sigma_2}$"},
        {x:"$\\Phi_{\\Sigma_1}+\\Phi_{\\Sigma_2}$"},
        {x:"$0$"}],
      hint:"Il flusso totale e\\u0300 la somma dei flussi sui pezzi, tutti uscenti.",
      sol:"Poiche\\u0301 $\\Phi_{\\partial V}=\\Phi_{\\Sigma_1}+\\Phi_{\\Sigma_2}+\\Phi_{\\Sigma_3}=\\iiint_V\\nabla\\cdot F$, si isola $\\Phi_{\\Sigma_3}=\\iiint_V\\nabla\\cdot F-\\Phi_{\\Sigma_1}-\\Phi_{\\Sigma_2}$. Tecnica del flusso 'per differenza', 20230210 Es.4. Teorema della divergenza."
    }]
  },

  { pool:"gauss", src:"tipo A · manuale", steps:[{
      t:"$F=\\dfrac{(x,y,z)}{(x^2+y^2+z^2)^{3/2}}$ ha $\\nabla\\cdot F=0$ dove definito. Il flusso uscente dalla sfera unitaria centrata nell'origine e\\u0300 $0$?",
      opts:[
        {x:"No: $F$ non e\\u0300 $C^1$ nell'origine, che sta dentro $V$; Gauss non si applica e il flusso vale $4\\pi$",c:true},
        {x:"Si\\u0300, perche\\u0301 $\\nabla\\cdot F=0$"},
        {x:"Si\\u0300, per ogni superficie chiusa"},
        {x:"No, ma il flusso e\\u0300 comunque $0$"}],
      hint:"Gauss richiede $F\\in C^1$ in TUTTO $V$, origine inclusa.",
      sol:"La divergenza e\\u0300 nulla solo fuori dall'origine, dove $F$ ha una singolarita\\u0300. Con l'origine dentro $V$ l'ipotesi $F\\in C^1(\\overline V)$ cade e Gauss non vale: il flusso e\\u0300 $4\\pi$ (campo di Coulomb). Teorema della divergenza, ipotesi di regolarita\\u0300."
    }]
  },
  ...
  { pool:"gauss", src:"tipo A · manuale", steps:[{
      ...
      opts:[
        ...
        {x:"un punto dove $F=0$"}],
      hint:"Il flusso su un piccolo volume attorno a $P$ e\\u0300 $\\approx\\nabla\\cdot F(P)\\cdot\\operatorname{vol}$.",
      sol:"$\\nabla\\cdot F(P)=\\lim_{V\\to P}\\frac{1}{\\operatorname{vol}(V)}\\iint_{\\partial V}F\\cdot n\\,dS$: e\\u0300 la densita\\u0300 di flusso uscente. Positiva = sorgente, negativa = pozzo. E\\u0300 la lettura fisica del Teorema della divergenza."
    }]
  },
  ...
  { pool:"gauss", src:"tipo A · manuale", steps:[{
      ...
      hint:"Serve $\\iiint_V\\nabla\\cdot F=\\operatorname{vol}(V)$, cioe\\u0300 divergenza unitaria.",
      sol:"Se $\\nabla\\cdot F=1$ allora $\\iint_{\\partial V}F\\cdot n\\,dS=\\iiint_V 1\\,dV=\\operatorname{vol}(V)$. Con $F=(x,y,z)$ la divergenza e\\u0300 $3$, quindi $\\operatorname{vol}(V)=\\tfrac13\\iint_{\\partial V}(x,y,z)\\cdot n\\,dS$. Teorema della divergenza."
    }]
  },

  { pool:"gauss", src:"tipo H · manuale", steps:[{
      t:"Scrivi $\\iint_\\Sigma F\\cdot n=\\iiint_V\\nabla\\cdot F$ per una calotta APERTA $\\Sigma$. Cosa c'e\\u0300 di sbagliato?",
      opts:[
        {x:"$\\Sigma$ non e\\u0300 chiusa: non borda alcun volume, quindi Gauss non e\\u0300 applicabile senza prima tapparla",c:true},
        {x:"niente, e\\u0300 corretto"},
        {x:"$V$ va preso bidimensionale"},
        {x:"manca solo un fattore $2\\pi$"}],
      hint:"Gauss lega un volume alla sua frontiera CHIUSA.",
      sol:"Una superficie aperta non e\\u0300 la frontiera di un solido: $\\iiint_V\\nabla\\cdot F$ non ha nemmeno un $V$ ben definito. Bisogna chiudere $\\Sigma$ con un tappo e poi sottrarne il flusso. Teorema della divergenza."
    }]
  },
  ...
  { pool:"gauss", src:"20220124 Es.3", steps:[{
      ...
      sol:"$\\Phi=\\iint_D(3+2y-x^2-y^2)(y-1)\\,dx\\,dy$. Con $y=1+\\rho\\sin\\theta$ il fattore $(y-1)=\\rho\\sin\\theta$ e\\u0300 dispari in $\\theta$ mentre l'altro fattore dipende da $\\rho$: l'integrale angolare $\\int_0^{2\\pi}\\sin\\theta\\,d\\theta=0$, quindi il flusso e\\u0300 $0$. 20220124 Es.3. Teorema della divergenza."
    }]
  },

  { pool:"gauss", src:"20230210 Es.4", steps:[{
      t:"$E=\\{x^2+y^2+z^2\\le25,\\;x^2+y^2\\le9\\}$ (sfera $\\cap$ cilindro), $F=(x,y,z)$. Per il flusso sul mantello cilindrico $\\Sigma_3$ la via piu\\u0300 rapida e\\u0300:",
      opts:[
        ...
        {x:"il flusso e\\u0300 $0$ per simmetria"}],
      hint:"Gauss da\\u0300 il totale; le calotte sono cartesiane e semplici, il mantello si ottiene per differenza.",
  ...
  { pool:"gauss", src:"tipo H · manuale", steps:[
    { t:"$\\Sigma$ e\\u0300 il paraboloide $z=x^2+y^2$ con $0\\le z\\le1$ (coppa APERTA), normale uscente dal solido sottostante. Chiudi con il disco $T=\\{x^2+y^2\\le1,\\,z=1\\}$. Con $F=(x,y,z)$, quanto vale $\\iiint_V\\nabla\\cdot F\\,dV$ sul solido $V$ fra paraboloide e disco?",
      opts:[
    ...
    { t:"Il flusso uscente attraverso il tappo $T$ (normale $(0,0,1)$, $F\\cdot n=z=1$) e\\u0300 l'area $\\pi$. Il flusso uscente attraverso la coppa $\\Sigma$ vale allora:",
      opts:[
      ...
      sol:"$\\iint_\\Sigma F\\cdot n\\,dS=\\iiint_V\\nabla\\cdot F\\,dV-\\iint_T F\\cdot n\\,dS=\\tfrac{3\\pi}{2}-\\pi=\\tfrac{\\pi}{2}$. Dimenticare il tappo (rispondere $\\tfrac{3\\pi}{2}$) e\\u0300 l'errore da evitare. Trucco 'tappare e Gauss' (tipo A/H). Teorema della divergenza." }
  ]},
  ...
  { pool:"gauss", src:"20230907 Es.3", steps:[{
      ...
      sol:"$\\iiint_T 4x\\,dV=\\int_0^1 4\\big(\\iint_{E_z}x\\,dA\\big)dz$. La sezione a quota $z$ e\\u0300 un disco di raggio $1-z$ centrato in $x=1-z$: $\\iint_{E_z}x\\,dA=(1-z)\\,\\pi(1-z)^2=\\pi(1-z)^3$. Flusso $=\\int_0^1 4\\pi(1-z)^3\\,dz=4\\pi\\cdot\\tfrac14=\\pi$. 20230907 Es.3. Teorema della divergenza."
    }]
  },

  { pool:"gauss", src:"tipo A · manuale", steps:[{
      t:"Flusso uscente di $F=(x,y,0)$ dal cilindro chiuso $\\{x^2+y^2\\le R^2,\\,0\\le z\\le h\\}$ (mantello piu\\u0300 due basi):",
      opts:[
        {x:"$2\\pi R^2 h$",c:true},
        {x:"$\\pi R^2 h$"},
        {x:"$0$ (perche\\u0301 $F_z=0$)"},
        {x:"$2\\pi R h$"}],
      hint:"$\\nabla\\cdot F=2$; sulle basi $F\\cdot(0,0,\\pm1)=0$, ma Gauss da\\u0300 comunque $2\\operatorname{vol}$.",
  ...
  { pool:"gauss", src:"tipo A · manuale", steps:[{
      ...
      sol:"$\\operatorname{vol}=\\int_0^{2\\pi}\\!\\!\\int_0^2\\!\\!\\int_0^{\\rho^2}\\rho\\,dz\\,d\\rho\\,d\\theta=\\int_0^{2\\pi}\\!\\!\\int_0^2\\rho^3\\,d\\rho\\,d\\theta=2\\pi\\cdot\\tfrac{16}{4}=8\\pi$. Il fattore $\\rho$ e\\u0300 il Jacobiano cilindrico: ometterlo darebbe $\\tfrac{8\\pi}{3}$ o simili. Volume come integrale triplo."
    }]
  },

  { pool:"gauss", src:"tipo H · manuale", steps:[{
      t:"Il tappo inferiore di un solido e\\u0300 il paraboloide $z=x^2+y^2$ (sotto), su $D=\\{x^2+y^2\\le1\\}$, con $F=(0,0,z)$. Per il flusso USCENTE da $V$: quale normale e quanto vale il flusso sul tappo?",
      opts:[
  ...
  { pool:"gauss", src:"tipo A · manuale", steps:[{
      t:"Il flusso uscente di $F=(x,y,z)$ da un solido $V$ vale $12$. Allora $\\operatorname{vol}(V)$ e\\u0300:",
      opts:[
        {x:"$4$",c:true},
        {x:"$12$"},
        {x:"$36$"},
